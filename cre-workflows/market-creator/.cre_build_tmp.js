...
function askGeminiForMarketIdea(runtime2, httpClient, geminiModel) {
...
{"question": "Will X happen by Y date?", "duration": 86400, "category": "crypto"}`;
  ...
  try {
    ...
    const idea = JSON.parse(generatedText);
    if (idea.question && idea.duration > 0) {
      return idea;
    }
    runtime2.log("[gemini] Invalid market idea format: " + generatedText);
    return null;
  } catch (e) {
    runtime2.log("[gemini] Failed to parse market idea: " + (e.message || e));
    return null;
  }
}
...
function onCronTrigger(runtime2) {
  ...
  runtime2.log("[market-creator] Cron triggered - searching for market ideas...");
  ...
  let idea = askGeminiForMarketIdea(runtime2, httpClient, cfg.geminiModel);
  if (!idea || !idea.question || idea.question.length < 10) {
    runtime2.log("[market-creator] Gemini unavailable, using fallback market idea");
    idea = {
  ...
  const duration = idea.duration > 0 ? idea.duration : 86400;
  runtime2.log(`[market-creator] Creating market: "${idea.question}"`);
  runtime2.log(`[market-creator] Category: ${idea.category}, Duration: ${duration}s`);
  const encodedParams = encodeAbiParameters(parseAbiParameters("string, uint256, uint256, bool, address"), [
    idea.question,
    BigInt(duration),
    BigInt(43200),
    true,
  ...
  const reportPayload = "0x00" + encodedParams.slice(2);
  ...
  runtime2.log("[market-creator] Report signed via consensus");
  ...
  runtime2.log(`[market-creator] Market created on-chain! TxHash: ${txHash.toString()}`);
  runtime2.log(`[market-creator] Question: "${idea.question}"`);
  return {
