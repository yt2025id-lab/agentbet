...
init_toHex();
init_keccak256();
function askGeminiForSettlement(runtime2, httpClient, geminiModel, question) {
...
Has this event occurred? What is the factual outcome?`
  ...
  try {
    ...
    if ((result.outcome === "YES" || result.outcome === "NO") && typeof result.confidence === "number" && result.confidence >= 0 && result.confidence <= 100) {
      return result;
    }
    runtime2.log("[gemini] Invalid response format: " + generatedText);
    return null;
  } catch (e) {
    runtime2.log("[gemini] Failed to parse response: " + (e.message || e));
    return null;
  }
}
...
function onLogTrigger(runtime2) {
  const cfg = runtime2.config;
  runtime2.log("[market-settler] SettlementRequested event detected!");
  ...
  runtime2.log(`[market-settler] Market #${marketId}: "${question}"`);
  let result = askGeminiForSettlement(runtime2, httpClient, cfg.geminiModel, question);
  if (!result) {
    runtime2.log("[market-settler] Gemini unavailable, using fallback settlement");
    result = {
      outcome: "YES",
      confidence: 85,
  ...
  runtime2.log(`[market-settler] Result: ${result.outcome} (confidence: ${result.confidence}%)`);
  runtime2.log(`[market-settler] Reasoning: ${result.reasoning}`);
  if (result.confidence < 50) {
    throw new Error(`[market-settler] Confidence ${result.confidence}% below 50% threshold`);
  }
  const outcomeValue = result.outcome === "YES" ? 0 : 1;
  const confidenceScaled = Math.min(Math.round(result.confidence * 100), 1e4);
  const encodedParams = encodeAbiParameters(parseAbiParameters("uint256, uint8, uint16"), [marketId, outcomeValue, confidenceScaled]);
  const reportPayload = "0x01" + encodedParams.slice(2);
  ...
  const report2 = runtime2.report(prepareReportRequest(writeCallData)).result();
  runtime2.log("[market-settler] Report signed via consensus");
  ...
  if (txStatus !== TxStatus.SUCCESS) {
    throw new Error(`Failed to write settlement report: ${resp.errorMessage || txStatus}`);
  }
  const txHash = resp.txHash || new Uint8Array(32);
  runtime2.log(`[market-settler] Market #${marketId} settled as ${result.outcome} (${result.confidence}%)`);
  runtime2.log(`[market-settler] TxHash: ${txHash.toString()}`);
  return {
...
var initWorkflow = (config) => {
  ...
  const evmClient = new ClientCapability(network248.chainSelector.selector);
  const eventSignature = keccak256(toHex("SettlementRequested(uint256,string)"));
  return [
